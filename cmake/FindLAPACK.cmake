...
macro(_lapack_find_dependency dep)
    # Special handling for BLAS - don't fail if it's being built
    if(${dep} STREQUAL "BLAS")
        # Check custom install locations first
        if(EXISTS "${CMAKE_INSTALL_PREFIX}/BLAS/lib/libopenblas.a")
            set(BLAS_LIBRARIES "${CMAKE_INSTALL_PREFIX}/BLAS/lib/libopenblas.a")
            set(BLAS_FOUND TRUE)
            set(BLAS_LIBRARY "${BLAS_LIBRARIES}")
            include_directories("${CMAKE_INSTALL_PREFIX}/BLAS/include")
            link_directories("${CMAKE_INSTALL_PREFIX}/BLAS/lib")
            message(STATUS "Found BLAS in custom install location: ${BLAS_LIBRARIES}")
            # BLAS is found, skip find_package
        elseif(EXISTS "${CMAKE_INSTALL_PREFIX}/BLAS/lib64/libopenblas.a")
            set(BLAS_LIBRARIES "${CMAKE_INSTALL_PREFIX}/BLAS/lib64/libopenblas.a")
            set(BLAS_FOUND TRUE)
            set(BLAS_LIBRARY "${BLAS_LIBRARIES}")
            include_directories("${CMAKE_INSTALL_PREFIX}/BLAS/include")
            link_directories("${CMAKE_INSTALL_PREFIX}/BLAS/lib64")
            message(STATUS "Found BLAS in custom install location: ${BLAS_LIBRARIES}")
            # BLAS is found, skip find_package
        else()
            # For BLAS, always use QUIET to avoid failing if it's being built
            find_package(${dep} ${ARGN} QUIET)
            if(NOT ${dep}_FOUND)
                # Don't fail immediately - BLAS might be building
                message(STATUS "BLAS not found yet, may be building...")
            endif()
        endif()
    else()
...
# Early check if BLAS is already installed via OpenBLAS in LAPACK directory
if(NOT BLAS_FOUND AND EXISTS "${CMAKE_INSTALL_PREFIX}/_deps/LAPACK/lib/libopenblas.a")
    message(STATUS "Found BLAS in LAPACK directory: ${CMAKE_INSTALL_PREFIX}/_deps/LAPACK/lib/libopenblas.a")
    set(BLAS_LIBRARIES "${CMAKE_INSTALL_PREFIX}/_deps/LAPACK/lib/libopenblas.a")
    set(BLAS_FOUND TRUE)
    set(BLAS_LIBRARY "${BLAS_LIBRARIES}")
    message(STATUS "Set BLAS_LIBRARIES: ${BLAS_LIBRARIES}")
endif()
...
if (NOT LAPACK_NOT_FOUND_MESSAGE)
    # Check if BLAS is already installed via OpenBLAS in LAPACK directory
    if(NOT BLAS_FOUND AND EXISTS "${CMAKE_INSTALL_PREFIX}/_deps/LAPACK/lib/libopenblas.a")
        message(STATUS "Found BLAS in LAPACK directory: ${CMAKE_INSTALL_PREFIX}/_deps/LAPACK/lib/libopenblas.a")
        set(BLAS_LIBRARIES "${CMAKE_INSTALL_PREFIX}/_deps/LAPACK/lib/libopenblas.a")
        set(BLAS_FOUND TRUE)
    ...
    # Only call _lapack_find_dependency if BLAS was not already found
    if(NOT BLAS_FOUND)
        # Check if BLAS was built in the custom install location (from BuildDependency)
        if(EXISTS "${CMAKE_INSTALL_PREFIX}/BLAS/lib/libopenblas.a")
            set(BLAS_LIBRARIES "${CMAKE_INSTALL_PREFIX}/BLAS/lib/libopenblas.a")
            set(BLAS_FOUND TRUE)
            set(BLAS_LIBRARY "${BLAS_LIBRARIES}")
        ...
        elseif(EXISTS "${CMAKE_INSTALL_PREFIX}/BLAS/lib64/libopenblas.a")
            ...
            # Try to find BLAS using standard CMake find module
