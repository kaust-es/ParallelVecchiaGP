...
namespace vecchia::kernels {
    template<typename T> bool BivariateMaternParsimonious<T>::plugin_name = plugins::PluginRegistry<vecchia::kernels::Kernel<T>>::Add(
            "BivariateMaternParsimonious", BivariateMaternParsimonious::Create);
}

template<typename T>
void BivariateMaternParsimonious<T>::GenerateCovarianceMatrix(double *apMatrixA, const int &aRowsNumber,
        ...
        for (j = 0; j < aColumnsNumber; j += 2) {
            ...
            if (expr == 0) {
                ...
                if (((i + 1) + j * aRowsNumber) < aRowsNumber * aColumnsNumber) {
                    apMatrixA[(i + 1) + j * aRowsNumber] = rho * sqrt(apLocalTheta[0] * apLocalTheta[1]);
                }
                if ((i + (j + 1) * aRowsNumber) < aRowsNumber * aColumnsNumber) {
                    apMatrixA[i + (j + 1) * aRowsNumber] = rho * sqrt(apLocalTheta[0] * apLocalTheta[1]);
                }
                ...
                apMatrixA[i + j * aRowsNumber] =
                        con1 * pow(expr, apLocalTheta[3]) * gsl_sf_bessel_Knu(apLocalTheta[3], expr);
                ...
                if (((i + 1) + (j + 1) * aRowsNumber) < aRowsNumber * aColumnsNumber) {
                    apMatrixA[(i + 1) + (j + 1) * aRowsNumber] =
                            con2 * pow(expr, apLocalTheta[4]) * gsl_sf_bessel_Knu(apLocalTheta[4], expr);
                }
            }
            j0++;
        }
