...
namespace vecchia::kernels {
    template<typename T> bool UnivariateMaternNonGaussian<T>::plugin_name = plugins::PluginRegistry<vecchia::kernels::Kernel<T>>::Add(
            "UnivariateMaternNonGaussian", UnivariateMaternNonGaussian<T>::Create);
}

template<typename T>
void UnivariateMaternNonGaussian<T>::GenerateCovarianceMatrix(double *apMatrixA, const int &aRowsNumber,
        ...
        for (j = 0; j < aColumnsNumber; j++) {
            ...
            if (expr == 0) {
                ...
                apMatrixA[i + j * aRowsNumber] =
                        con * pow(expr, apLocalTheta[1]) * gsl_sf_bessel_Knu(apLocalTheta[1], expr);
            }
            j0++;
        }
        i0++;
