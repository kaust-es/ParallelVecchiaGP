...
namespace vecchia::kernels {
    template<typename T> bool UnivariateMaternDdsigmaSquareNu<T>::plugin_name = plugins::PluginRegistry<vecchia::kernels::Kernel<T>>::Add(
            "UnivariateMaternDdsigmaSquareNu", UnivariateMaternDdsigmaSquareNu<T>::Create);
}

template<typename T>
void UnivariateMaternDdsigmaSquareNu<T>::GenerateCovarianceMatrix(double *apMatrixA, const int &aRowsNumber,
