...
namespace vecchia::kernels {
    template<typename T> bool UnivariateMaternDbeta<T>::plugin_name = plugins::PluginRegistry<vecchia::kernels::Kernel<T>>::Add(
            "UnivariateMaternDbeta", UnivariateMaternDbeta<T>::Create);
}

template<typename T>
void UnivariateMaternDbeta<T>::GenerateCovarianceMatrix(double *apMatrixA, const int &aRowsNumber, const int &aColumnsNumber,
