...
namespace vecchia::kernels {
    template<typename T> bool UnivariateMaternNuggetsStationary<T>::plugin_name = plugins::PluginRegistry<vecchia::kernels::Kernel<T>>::Add(
            "UnivariateMaternNuggetsStationary", UnivariateMaternNuggetsStationary<T>::Create);
}

template<typename T>
void UnivariateMaternNuggetsStationary<T>::GenerateCovarianceMatrix(double *apMatrixA, const int &aRowsNumber,
