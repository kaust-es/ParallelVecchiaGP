...
namespace vecchia::kernels {
    template<typename T> bool UnivariateMaternStationary<T>::plugin_name = plugins::PluginRegistry<vecchia::kernels::Kernel<T>>::Add(
            "UnivariateMaternStationary", UnivariateMaternStationary<T>::Create);
}

template<typename T>
void
UnivariateMaternStationary<T>::GenerateCovarianceMatrix(double *apMatrixA, const int &aRowsNumber, const int &aColumnsNumber,
